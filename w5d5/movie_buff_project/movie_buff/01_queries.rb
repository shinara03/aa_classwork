...
def harrison_ford
  ...
  # Actor.joins(:movies, :castings).select("movies.id, movies.title").where("actors.name= 'Harrison Ford' AND castings.ord != 1").group("movies.id")
  #.where.not("castings.ord = 1")
  Movie.joins(:actors).select("movies.id, movies.title").where("actors.name= 'Harrison Ford' AND castings.ord != 1").group("movies.id")
...
def biggest_cast
  ...
  Movie.joins(:actors).select("movies.id, movies.title").group("movies.id").order("COUNT(actors.id) DESC").limit(3)
end
